...
N_KV_HEADS = 8      # Key/Value 头总数 (GQA)
D_MODEL = 4096      # 隐藏层维度
D_HEAD = 128        # 每个头的维度
GROUP_SIZE = N_Q_HEADS // N_KV_HEADS   # 每组包含的 Q 头数量 (4)

# 分析所有层 (0~31)
LAYERS_TO_ANALYZE = set(range(32))

# 自动查找所有分片 (按数字顺序排序)
file_pattern = "llama-3-8b-model-*.safetensors"
file_list = sorted(glob.glob(file_pattern))
if not file_list:
    raise FileNotFoundError(f"未找到匹配 '{file_pattern}' 的文件，请检查路径。")
print(f"找到 {len(file_list)} 个分片: {file_list}")

# ---------------------------- 辅助函数 ----------------------------
def extract_layers_from_keys(keys):
    """从键名中提取所有层号 (例如 model.layers.12.self_attn.q_proj.weight)"""
    layers = set()
    for k in keys:
...
def key_exists_for_layer(keys, layer_idx, proj_type):
    """检查某个分片中是否存在指定层和投影类型 (q_proj/k_proj) 的权重"""
    pattern = f"layers.{layer_idx}."
    if proj_type == "q":
        candidates = ["q_proj", ".wq"]
    else:
        candidates = ["k_proj", ".wk"]
    for k in keys:
        if pattern in k and any(c in k for c in candidates):
            return True
    return False

def get_key_for_layer(keys, layer_idx, proj_type):
    """返回指定层和投影类型的完整键名"""
    pattern = f"layers.{layer_idx}."
    if proj_type == "q":
        candidates = ["q_proj", ".wq"]
...
# ---------------------------- 主流程：逐分片加载 ----------------------------
print("\n开始逐分片加载并分析...")
print(f"目标层范围: {min(LAYERS_TO_ANALYZE)} - {max(LAYERS_TO_ANALYZE)}")
...
for file_idx, fpath in enumerate(file_list, 1):
    ...
    layers_in_this_file = extract_layers_from_keys(keys)
    print(f"  该分片包含的层: {layers_in_this_file}")

    # 找出需要在该分片中分析的层（未分析过且在目标范围内）
    to_analyze = [L for L in layers_in_this_file if L in LAYERS_TO_ANALYZE and L not in analyzed_layers]
    if not to_analyze:
    ...
    # 对每个需要分析的层，提取 Q 和 K 的权重张量
    for L in to_analyze:
        q_key = get_key_for_layer(keys, L, "q")
        k_key = get_key_for_layer(keys, L, "k")
        if q_key is None or k_key is None:
            print(f"    层 {L} 缺少 Q 或 K 键，跳过")
            continue
        wq = weights[q_key]   # 留在CPU，稍后传到GPU
        wk = weights[k_key]
...
# 检查是否所有层都已分析
missing = LAYERS_TO_ANALYZE - analyzed_layers
if missing:
    print(f"\n警告：以下层未找到权重文件中的对应键：{sorted(missing)}")
else:
    print("\n✅ 所有目标层分析完成。")
