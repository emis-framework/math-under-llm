...
**王氏五定律（Wang's Five Laws）** 提供了一个 **静态、可复现的框架**，用于仅通过注意力权重评估 LLM 的推理能力。
...
根据我们已有的数据，以下是第四定律和第五定律的README内容，格式与第一、第二定律一致。
...
**说明：**  
注意力机制中 Q、K、V 矩阵的左奇异向量（输出子空间）之间存在系统性的几何结构：

- **Q 与 K 的输出方向**接近随机正交，保证查询与键的功能分离。
- **Q 与 V、K 与 V 的输出方向**显著低于随机正交（超正交），确保检索路径与内容读出路径的通道隔离，防止信息短路。

**数学表达：**

设 $U_Q, U_K, U_V \in \mathbb{R}^{d_h \times d_h}$ 分别为 Q、K、V 的左奇异向量矩阵，其对应列的绝对余弦相似度均值定义为：
...
$$
\overline{\cos}(U_Q, U_K) \sim \frac{1}{\sqrt{d_h}} \quad \text{(近似随机正交)}
$$

$$
\overline{\cos}(U_Q, U_V) < \frac{1}{\sqrt{d_h}}, \quad \overline{\cos}(U_K, U_V) < \frac{1}{\sqrt{d_h}} \quad \text{(超正交)}
$$

**实测数据（跨模型均值）：**

| 模型                         | $d_h$ | 随机基准 | $\overline{\cos}(U_Q,U_K)$ | $\overline{\cos}(U_Q,U_V)$ | $\overline{\cos}(U_K,U_V)$ |
| ---------------------------- | ----- | -------- | -------------------------- | -------------------------- | -------------------------- |
...
| Gemma‑4‑31B (文本)           | 256   | 0.0625   | 0.0630                     | **0.0497**                 | **0.0500**                 |
| Gemma‑4‑31B (视觉)           | 128   | 0.0884   | 0.1024                     | **0.0714**                 | **0.0713**                 |

> **超正交**定义为余弦值系统性地低于随机正交期望，通常偏离幅度约 **20%**。该现象在所有测试模型和模态中一致存在，表明预训练会主动将 Value 的输出子空间推离 Q/K 的输出子空间。

---

### 5️⃣ 第五条定律 — 输入子空间全局随机正交

**说明：**  
Q、K、V 的右奇异向量（输入子空间）在高维 token 空间中的对齐程度极其接近随机期望，不存在结构性耦合。这意味着模型从输入嵌入中自由地、独立地选择敏感方向，没有强制性的共享基底。

**数学表达：**

设 $V_Q, V_K, V_V \in \mathbb{R}^{d_{\text{model}} \times d_h}$ 分别为 Q、K、V 的右奇异向量矩阵，其对应列的绝对余弦相似度均值：

$$
\overline{\cos}(V_A, V_B) = \frac{1}{d_h} \sum_{i=1}^{d_h} |\langle v_{A,i}, v_{B,i} \rangle|
$$

与随机基准 $1/\sqrt{d_{\text{model}}}$ 相比：
...
| 模型                         | $d_{\text{model}}$ | 随机基准 | $\overline{\cos}(V_Q,V_K)$ | $\overline{\cos}(V_Q,V_V)$ | $\overline{\cos}(V_K,V_V)$ |
| ---------------------------- | ------------------ | -------- | -------------------------- | -------------------------- | -------------------------- |
...
| Gemma‑4‑31B (文本)           | 5376               | 0.0136   | 0.0167                     | 0.0128                     | 0.0153                     |
| Gemma‑4‑31B (视觉)           | 1152               | 0.0295   | **0.0440**                 | 0.0306                     | 0.0304                     |

> - 在标准文本 Transformer 中，`cosV_QK` 值的微小升高（约 1.5 倍基准）是一种 **极弱的输入耦合**，远未达到“结构性对齐”的程度，可视为随机正交的自然波动。  
> - 视觉编码器中 `cosV_QK` 明显升高（0.044 vs 0.030），反映了视觉自注意力需要共享空间敏感方向的特殊需求，但不改变 V 空间整体接近随机正交的基本结论。

---

**定律总结**  
这五条定律共同描绘了 Transformer 注意力的**静态谱结构**：  
- **Σ 空间**（奇异值）：完全对齐（第一、第二定律）  
- **U 空间**（输出）：随机正交 + 超正交，保证功能解耦（第四定律）  
- **V 空间**（输入）：全局随机正交，保证特征自由选择（第五定律）

该结构在多个模型（Qwen、DeepSeek‑R1、LLaMA‑3、Gemma‑4）、多模态（视觉/文本）下均保持高度一致，表明它是预训练收敛到的**普适几何不动点**。
...
我们的发现表明，**你无需运行任何 benchmark 就能评估 LLM 的推理质量**——只需分析静态的权重矩阵即可。这在以下方向具有直接的应用价值：

- **训练效率**：省下无效的计算——通过谱指标而非仅看 loss 曲线来判断收敛
- **模型合并**：使用 SSR 作为适应度函数，在可证明保持推理能力的前提下合并模型
- **量化压缩**：通过感知 SSR 的混合精度策略，实现 2–4 倍模型压缩且推理能力零衰退
- **微调防遗忘**：用 SSR 正则化防止灾难性遗忘

详细的应用分析与理论方向，请参阅我们的[**白皮书 → 第7节**](WHITEPAPER.cn.md#7-工程应用与未来方向)
...
王氏五定律不仅是理论发现，更是一套可直接嵌入工程流程的诊断工具。本章汇总五定律在模型训练、推理部署、架构设计、安全防御等关键领域的应用前景。
