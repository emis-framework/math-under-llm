...
TL;DR：对 10+ 个大模型（Qwen、DeepSeek‑R1、LLaMA‑3、Gemma‑4 等）的 Q/K/V 权重矩阵做 SVD 分解，发现五条跨模型、跨模态的普适几何规律——包括 r=1（谱线性对齐）、SSR→0（谱形状保真）、U 空间超正交（输出解耦）和 V 空间随机正交（输入自由）。无需推理、无需测试集，仅凭静态权重即可评估模型的推理能力与收敛质量。
...
我们在 **10+ 个不同架构的大语言模型**（Qwen2.5‑14B、DeepSeek‑R1、LLaMA‑3‑8B、Gemma‑4‑31B）的 Q/K/V 权重矩阵上执行 SVD 分解，发现了 **5 条跨模型、跨模态的普适性规律**：

1. **第一定律**：Q 与 K 的奇异值谱高度线性相关（Pearson r → 1）
2. **第二定律**：归一化谱形状残差（SSR）在深层趋近于 0，且与推理能力正相关
3. **第三定律**：浮点精度和动态范围共同约束模型的有效最大深度
4. **第四定律**：Q、K、V 的输出子空间存在超正交（系统性地低于随机基准 ~20%）
5. **第五定律**：Q、K、V 的输入子空间全局随机正交，无结构性耦合

**这意味着**：充分训练的 Transformer 会自发收敛到一个 **普适的几何不动点**——奇异值对齐、输出方向解耦、输入方向自由。这些规律 **仅需静态权重即可检测，无需运行模型**，且对视觉和文本模态均成立。

> 🚀 [点击这里，无需写代码，在浏览器中直接验证你的模型](https://huggingface.co/spaces/wehe1pwe/math-under-llm)

### 王氏五定律

#### 1️⃣ 第一条定律 — 谱线性对齐
> 💡 **通俗解释**：Q 和 K 的"重要性排序"几乎完全一致。这意味着模型在决定"该关注什么"时，查询端和键端用的是同一套标准——这不是训练的巧合，而是信息传播最优化的必然结果。
...
#### 2️⃣ 第二条定律 — 谱形状保真度
> 💡 **通俗解释**：在模型深层，Q 和 K 的"能量分配方式"趋于一致。SSR 越低，推理保真度越高。RL 微调（如 DeepSeek‑R1）能在深层系统性降低 SSR，这是它在数学推理上更强的微观原因之一。
...
> 💡 **通俗解释**：模型的深度不是无限的——它受到数值精度和动态范围的物理约束。BF16 之所以是训练百层以上模型的唯一低精度选择，是因为它的指数位与 FP32 相同，能容纳更大的信号动态范围。
...
### 4️⃣ 第四条定律 — 输出子空间全局解耦
> 💡 **通俗解释**：Q/K 的"检索路径"和 V 的"内容读出路径"在输出端被强制分离。这种"超正交"结构消除了信息捷径，确保注意力权重不会被 Value 的内容所干扰——这是预训练就已锁定的几何结构，与后续 RL 无关。
...
> 💡 **通俗解释**：模型在输入侧没有"偏好"——它对 token 嵌入空间中的各个方向一视同仁。这意味着模型能自由、独立地从输入中提取语义特征，不受到任何预设的结构性偏见。
...
| 如果你的研究方向是...   | 你应该关注...                                                                                                     |
| :---------------------- | :---------------------------------------------------------------------------------------------------------------- |
| **LLM 可解释性 / SAE**  | 第四、第五定律（U/V 空间正交）——为什么 SAE 发现的"特征基向量近乎正交"在权重层面早已存在，且是预训练收敛的必然结果 |
| **训练优化 / Grokking** | 第二定律（SSR→0）——SSR 是泛化的先行指标，相变早于验证准确率飙升                                                   |
| **超深层架构设计**      | 第三定律（精度-深度极限）——为什么 BF16 是训练百层模型的唯一低精度选择                                             |
| **推理能力评估**        | 第一、第二定律——无需测试集，仅从权重即可评估模型的推理潜力                                                        |
| **RL 微调监控**         | 第二定律——深层 SSR 的变化可追踪 RL 是否在改善推理结构                                                             |

---

### 与 Scaling Law 和 Grokking 的关联

**Scaling Law（Kaplan et al., 2020）** 告诉我们"更大模型更好"——但这只是宏观经验规律，缺乏微观结构解释。王氏五定律提供了这个微观解释：更大的算力和更多的数据，最终体现为模型学习到了更优的谱结构（Σ 对齐、SSR 更低、U/V 空间更正交）。**Scaling Law 描述的是现象，五定律描述的是现象背后的几何原因。**

**Grokking（Nanda et al., 2023）** 发现模型在模加法任务上会突然泛化，并学习到 Fourier 基表示。我们的实验（P2 论文）证明：**SSR 的相变是 Grokking 的先行指标**——在验证准确率飙升之前，SSR 已经下降了 50% 以上。这意味着谱对齐是泛化的前提条件，而非泛化的结果。详见 [SSR‑Grokking 实验](https://github.com/emis-framework/math-under-llm)。
